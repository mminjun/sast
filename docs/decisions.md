...
- **성능 — taint 7룰과 `--dataflow-traces --text-output`을 더해도 스캔 시간은 달라지지 않는다** | 패턴만
  ...
  | 이 저장소 (CI 제외 적용) | 12.9초 | 11.3초 | 1 → 7 (아래) |

  600초 타임아웃 안에서 여유가 크다 | SEC-009
- **자체 저장소 도그푸딩 — 매개변수 소스의 비용이 숫자로 드러났다: IV-03 7건, 전부 "경로를 매개변수로
  받아 여는 헬퍼"** | `read_snippet(path)`, `load_trace_file(file_path)`, zip 추출의 `open(fs_path(target))`,
  `scripts/sast_gate.py`의 CLI 인자 파일 열기. 호출자가 격리 루트 검사(`relative_to`)를 마친 경로를 넘기지만
  함수 안에서는 그것을 알 수 없으니 매개변수 소스 규칙상 걸린다 — 이것이 "매개변수를 소스로 본다"의
  정확한 비용이다. 처리: (1) `$P.open(...)` 싱크는 뺐다(타입을 모르는 Semgrep에선 `ZipFile.open(member)`
  같은 무관한 `.open()`이 걸려 2건이 순수 오탐이었다). (2) 나머지 7건은 룰을 더 좁히지 않고 오탐 판정
  워크플로로 다룬다 — IV-03만 요청 값 소스로 좁히면 `read_report(name): open("/var/reports/" + name)`
  같은 정탐도 함께 사라진다. CI 게이트는 base·head를 같은(head) 룰셋으로 스캔하므로 이 7건은 '유지'로
  집계돼 게이트를 막지 않는다. 같은 zip으로 패턴 룰 실행(run 53)과 taint 룰 실행(run 57)을 비교하니
  new 0 / resolved 0 / persisted 14 — IV-01·02·05·07 결과가 전부 '유지'로 짝지어졌다(핑거프린트가 KISA
  코드 기준이라는 판단 2의 실증) | SFR-009, TST-005
