...
| SFR-009 | 분석 처리 체계 | ✅ | analysis/services.py `run_semgrep` + django.tasks(DB 백엔드 django-tasks-db) | Semgrep subprocess를 워커 프로세스에서 실행(9/6, 큐는 PostgreSQL 테이블 — Redis 등 새 인프라 없음, Celery 미채택 근거는 decisions.md 9/6). 룰셋은 자체 KISA 룰(`catalog/rules/`) — 인젝션 계열 Python 7항목은 taint 모드(`taint_python.yaml`, 9/6), 오염 경로는 `--dataflow-traces --text-output`으로 작업 영역에 받아 표준화가 읽음. 자체 taint 엔진(`analysis/taint`, Django 비의존, `execute_analysis`가 Semgrep 다음에 실행, 9/6 1단계 함수 내 — Semgrep taint와 동등성 실증; 2단계 같은 파일 함수 간 — 함수 요약 고정점, Semgrep이 못 잡는 4건(이름 규약 거짓말·헬퍼 안 입력)과 Semgrep 오탐 5건 제거를 시험으로 고정; 클래스 필드는 3단계). `--no-git-ignore`·`PYTHONUTF8=1`·`encoding='utf-8'` 필요 (docs/decisions.md 외부 도구 연계 문제) |
