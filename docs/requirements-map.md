...
| SFR-009 | 분석 처리 체계 | ✅ | analysis/services.py `run_semgrep` + django.tasks(DB 백엔드 django-tasks-db) | Semgrep subprocess를 워커 프로세스에서 실행(9/6, 큐는 PostgreSQL 테이블 — Redis 등 새 인프라 없음, Celery 미채택 근거는 decisions.md 9/6). 룰셋은 자체 KISA 룰(`catalog/rules/`) — 인젝션 계열 Python 7항목은 taint 모드(`taint_python.yaml`, 9/6), 오염 경로는 `--dataflow-traces --text-output`으로 작업 영역에 받아 표준화가 읽음. 자체 taint 엔진(`analysis/taint`, Django 비의존, `execute_analysis`가 Semgrep 다음에 실행, 9/6 1단계 함수 내 — Semgrep taint와 동등성 실증; 2단계 같은 파일 함수 간 — 함수 요약 고정점, Semgrep이 못 잡는 4건(이름 규약 거짓말·헬퍼 안 입력)과 Semgrep 오탐 5건 제거를 시험으로 고정; 3단계 클래스 필드 — 다른 메서드에서 대입된 필드·`__init__` 인자·필드 체인, Semgrep이 못 잡는 5건·오탐 1건 제거 고정, 흐름 비민감의 대가 실측(Django 906파일 15건·순서 오탐 0). 계획한 3단계 완료). `--no-git-ignore`·`PYTHONUTF8=1`·`encoding='utf-8'` 필요 (docs/decisions.md 외부 도구 연계 문제) |
...
| SFR-011 | 초기 분석 대상 지원 | ✅ | catalog/rules/{c,java,js}_*.yaml, `catalog/samples/vulnerable.{c,java,js}`·`safe.*`, settings `ANALYSIS_SCAN_TARGET_SUFFIXES` | 완료(9/5 판정, decisions.md) — Python·C·Java·JS/TS 4개 언어 구현: 9/4 C 룰 13개(신규 실탐지 6개), 9/5 Java 35룰·JS 30룰(신규 실탐지 12개, 합계 39/49). 확장자 .py/.c/.h/.java/.js/.jsx/.ts/.tsx. 9/6 Python 인젝션 7항목(IV-01·02·03·04·05·07·12)을 Semgrep taint 모드로 교체 — 변수 경유 흐름을 잡고 상수만 흐르면 안 잡음, IV-04 Python 커버리지 신설. Java·JS taint는 다음 브랜치. Semgrep 무료 엔진 한계(함수·파일·클래스 필드 간 추적 없음)는 decisions.md 9/6 taint 표 |
...
| SFR-014 | 분석 결과 표준화 | ✅ | catalog/services.py `ingest_findings` + `catalog/taint_trace.py` | raw_result → Finding. 실행 성공 시 시그널로 자동 연결, 멱등. 관리자용 재표준화 API 별도. 9/6: `extra.engine`(semgrep-pattern/semgrep-taint/예약 custom-taint)과 `extra.taint_trace`(소스→경유→싱크, 노드마다 path·line·code — 자체 taint 구현이 같은 형식을 씀). 오염 경로는 Semgrep 텍스트 출력 어댑터로 읽고, 파싱 실패 시 탐지 결과는 저장·경로만 비우고 경고 로그. 자체 taint 결과(`custom_result`)를 (KISA 코드·파일·싱크 줄)로 병합 — 겹치면 custom-taint를 남기고 `extra.engines`에 양쪽 기록, `stats.semgrep_only`가 회귀 신호(경고 로그·실행 상세 `custom_taint_stats`) |
...
| QLT-002 | 진단 항목 독립성 | ✅ | 룰은 `catalog/rules/*.yaml`에 독립. `metadata.kisa_code`가 룰↔항목 매핑의 단일 원본이고, 시드 커맨드가 스캔해 반영. `metadata.engine`은 허용 목록으로 검증(9/6) |
